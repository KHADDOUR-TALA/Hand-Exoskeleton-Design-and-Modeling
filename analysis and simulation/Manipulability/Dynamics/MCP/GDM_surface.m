...
for i = 1:length(L1_range)
    for j = 1:length(L4_range)
        ...
        global_manipulability_surface(j, i) = computeGlobalManipulability(L, Alpha1, Theta, W);
    end
end
...
m = mean(global_manipulability_surface);
m = mean(m);
%%n = mean(W_v, 'all');
...
figure;
surf(LL1, LL4, global_manipulability_surface);
xlabel('L1');
ylabel('L4');
zlabel('Global Manipulability');
